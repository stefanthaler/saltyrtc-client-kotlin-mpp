package deprecated.signalling.messages
...
abstract class SignallingMessage(val nonce: Nonce) {
    ...
    companion object {
        fun parse(dataBytes:ByteArray, nonceBytes:ByteArray, client: DeprecatedSaltyRTCClient, naCl:NaCl?): IncomingSignallingMessage {
...
abstract class IncomingSignallingMessage: SignallingMessage {
     ...
     */
    abstract fun validate(client: DeprecatedSaltyRTCClient, payloadMap: Map<String, Any>)

    fun validateSource(client: DeprecatedSaltyRTCClient) { //TODO this is for incoming messages only
        require(nonce.source.toInt()==0)
    }

    open fun validateDestination(client: DeprecatedSaltyRTCClient) { //TODO this is for incoming messages only
    ...
    constructor(nonce: Nonce, client: DeprecatedSaltyRTCClient, payloadMap: Map<String, Any>):super(nonce){
...
abstract class OutgoingSignallingMessage(
    ...
    suspend fun send(client: DeprecatedSaltyRTCClient, naCl: NaCl?=null) {
     ...
     * @param naCl
     */
    fun toByteArray(client: DeprecatedSaltyRTCClient, naCl: NaCl?=null):ByteArray {
        validate(client)
    ...
    abstract fun validate(client: DeprecatedSaltyRTCClient)
}
